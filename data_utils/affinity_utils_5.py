...
def getKeypoints(mask, thresh=0.8, is_gaussian=True, is_skeleton=False, smooth_dist=4):
    ...
    if is_skeleton:
        ...
        # ske,dis = medial_axis(image=mask,return_distance=True)
        # ske = ske.astype(np.uint16)
        # dist_on_skel = dis * ske
        # exist = np.nansum(dist_on_skel != 0)
        # dist_on_skel = np.nansum(dist_on_skel)
        # if exist == 0:
        #     exist = 1
        # mean_value = dist_on_skel / exist
    ...
    for line in linestrings:
        ...
        if "EMPTY" in points_str:
            # return keypoints,np.nanmean(mean_value)
            return keypoints
        points = []
...
def computeRoadtheta(mask, thresh=0.8, is_gaussian=True):
    if is_gaussian:
        # mask /= 255.0
        mask[mask < thresh] = 0
        mask[mask >= thresh] = 1
    ...
    exist = (dist_on_skel != 0)
    if exist.sum() == 0 :
        mean_value = 0.0
    else:
        mean_value = dist_on_skel.sum() / exist.sum()
    return mean_value

def get_SemanticRoad_Edge(mask, thresh=0.8):
    """
    Generate keypoints for binary prediction mask.
    ...
    img1 = cv2.Sobel(mask, cv2.CV_64F, 1, 0)
    sobelx = cv2.convertScaleAbs(img1)

    img1 = cv2.Sobel(mask, cv2.CV_64F, 0, 1)
    sobely = cv2.convertScaleAbs(img1)

    edge = cv2.addWeighted(sobelx, 0.5, sobely, 0.5, 0)
    # edge = cv2.Canny(mask, 0.3, 0.8) / 255.0
    # gaussImg = cv2.Sobel(img,cv2.CV_64F, 0, 1)
    edge[edge < thresh] = 0
    edge[edge >= thresh] = 1

    for x in range(0, edge.shape[0]):
        for y in range(0, edge.shape[1]):
...
def getRoadedge(mask, thresh=0.8):
    """
    Generate keypoints for binary prediction mask.
    ...
    """
    mask = mask.astype(np.uint8)
    edge = cv2.Canny(mask,thresh,thresh).astype(float)
    edge[edge < thresh] = 0.0
    edge[edge >= thresh] = 1.0
    return edge

def get_PredictRoad_Edge(output):

    x_size = output.size()  #获取输入图的size
    output_arr = output.cpu().detach().numpy().astype(np.uint8)

    #进行维度交换，便于下一步进入CV库进行运算
    edge =  np.zeros((x_size[0], x_size[1], x_size[2], x_size[3]))     #初始化一个edge 4维数组
    for i in range(x_size[0]):
        for j in range (x_size[1]):
            img1 = cv2.Sobel(output_arr[i,j], cv2.CV_64F, 1, 0)
            sobelx = cv2.convertScaleAbs(img1)

            img1 = cv2.Sobel(output_arr[i,j], cv2.CV_64F, 0, 1)
            sobely = cv2.convertScaleAbs(img1)

            edge[i,j] = cv2.addWeighted(sobelx, 0.5, sobely, 0.5, 0)/255.0
            # edge[i,j] = cv2.Canny(output_arr[i,j],50,200)/255.0
    edge = torch.from_numpy(edge).cuda().float()

    return edge

def getVectorMapsAngles(shape, keypoints,theta,bin_size=10):
    """
    ...
    """
    # theta = computeRoadtheta(mask, thresh=0.8, is_gaussian=True)
    im_h, im_w = shape
    ...
    vecmap_angles = (vecmap_angles / bin_size).astype(float)
